...
import {
  ...
  setOnlineStatus,
  setVideoAccess,
} from "@/lib/redux/state/ExamEligibilityTest";
...
function ExamWrapper({
  ...
  const isEligible = useSelector(
    (state: RootState) => state.exam_eligibility_test.is_eligible,
  );
  ...
  useEffect(() => {
    const local_storage = JSON.parse(
      localStorage.getItem("user_answers") || "{}",
    );
    let data: object = {};
    ...
    setIsProctored(exam_questions.attempt.exam.is_proctored);
  }, []);
  ...
  if (isProctored && !isEligible) {
    return (
      <EligibilityTest setStartExam={() => dispatch(setIsEligible(true))} />
    );
  }
